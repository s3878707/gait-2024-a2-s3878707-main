...
public class AStarGrid : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                bool walkable = (
                    Physics2D.OverlapCircle(worldPoint, overlapCircleRadius, unwalkableMask) == null
                );
                bool underwater = (
                    Physics2D.OverlapCircle(worldPoint, overlapCircleRadius, waterMask) != null
                );
                grid[x, y] = new Node(walkable, worldPoint, x, y, underwater);
            }
        }
    }
    ...
    public List<Node> GetNeighbours(Node node)
    {
        ...
        // Include diagonal neighbours if the flag is true
