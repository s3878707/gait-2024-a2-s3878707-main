...
public class Frog : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ...
            bubble.GetComponent<Rigidbody2D>().velocity =
                _rb.velocity + 5.0f * (Vector2)transform.up.normalized;
        }
    ...
    void FixedUpdate()
    {
        ...
        if (_lastClickPos != null)
        {
            ...
            )
            {
                ...
                Vector2 currentWaypoint = path[targetIdx].worldPosition;
                ...
                {
                    desiredVel = Steering.BasicSeek(
                        gameObject.transform.position,
                        currentWaypoint,
                        MaxSpeed
                    );
                }
                if (distanceToWaypoint <= 1f && targetIdx < path.Length)
                {
