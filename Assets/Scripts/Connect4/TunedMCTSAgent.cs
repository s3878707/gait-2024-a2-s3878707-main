...
public class TunedMCTSAgent : Agent
{
    ...
    public float Simulation(Connect4State state)
    {
        ...
        // Prioritize moves that lead to winning positions or block opponent's winning moves
        ...
        while (gameResult == Connect4State.Result.Undecided)
        {
            ...
            {
                int bestMove = GetBestMove(state, possibleMoves);
                if (bestMove != -1)
                {
                ...
                {
                    int randomMove = possibleMoves[
                        UnityEngine.Random.Range(0, possibleMoves.Count)
                    ];
                    state.MakeMove(randomMove);
                    gameResult = state.GetResult();
                }
    ...
    private int GetBestMove(Connect4State state, List<int> possibleMoves)
    {
        foreach (int move in possibleMoves)
        {
            Connect4State nextState = state.Clone();
            nextState.MakeMove(move);
            Connect4State.Result gameResult = nextState.GetResult();
            if (gameResult != Connect4State.Result.Undecided)
                return move;
        }
